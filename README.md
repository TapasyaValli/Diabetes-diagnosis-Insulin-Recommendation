...
The classifier uses the final important-feature set below. `Random_Glucose` was removed because leave-one-feature-out testing showed no accuracy loss, so it was redundant with `HbA1c` and `Fasting_Glucose`.
...
`Weight_kg` is also reused by the insulin recommender for the clinical total daily dose formula.
